...
DEFAULT_REQUEST_TIMEOUT = 60  # Seconds before request times out
...
def authenticate_user():
    """Authenticate user with username/password and get API key"""
    print("="*63)
...
def get_workspace_selection(api_key):
    """Get workspace selection from user"""
    print("\n" + "="*63)
...
def get_monitoring_preferences():
    """Get monitoring preferences from user"""
    print("\n" + "="*63)
...
def save_session_config(api_key, check_interval, csv_dir, workspace_id, workspace_name):
    """Save session configuration to a temporary file"""
    script_dir = get_script_directory()
    # Create unique session file per workspace for multi-instance support
...
def load_session_config(workspace_name=None):
    """Load session configuration if it exists and is recent"""
    script_dir = get_script_directory()
    
    # If workspace_name provided, look for specific session file
    if workspace_name:
        safe_workspace_name = "".join(c for c in workspace_name if c.isalnum() or c in (' ', '-', '_')).rstrip()
        safe_workspace_name = safe_workspace_name.replace(' ', '_').lower()
        session_file = os.path.join(script_dir, f".session_config_{safe_workspace_name}")
    else:
        # Look for any existing session files
    ...
    try:
        ...
        # If there's any error reading the session, remove it
        try:
            os.remove(session_file)
        except:
            pass
...
def check_running_instances():
    """Check for other running monitor instances"""
    script_dir = get_script_directory()
...
def initialize_configuration():
    """Initialize configuration through authentication or session"""
    args = parse_arguments()

    # Check for other running instances
    check_running_instances()
    
    # Check for existing session unless reset requested
...
def send_csv_file(file_path):
    """
    Send a CSV file to the API endpoint
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                # Count prediction types for summary
...
def process_csv_directory():
    ...
    for csv_file in csv_files:
        ...
        # Skip if already exists in processed directory
        ...
        # If failed, move to failed directory
...
def main():
    """Main function - monitors directory and processes files"""
    display_header()
